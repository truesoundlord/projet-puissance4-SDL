...
unsigned short jetonsRouges=(MAXCOLS*MAXLINS)/2;
unsigned short jetonsJaunes=(MAXCOLS*MAXLINS)/2;
...
int main(int argc, char *argv[])
{
  if(argc>3 || argc<2 || argv[1]==NULL)
  {
	...
	/****************************************************************************
	 * ATTENTION !!!!!
	 * Pour les tests veiller à ce que les interfaces virtuelles vmware
  ...
  else
  {
	...
	for(cptY=0;cptY<MAXLINS;cptY++)
		for(cptX=0;cptX<MAXCOLS;cptX++)
			tabPuissance4[cptY][cptX]=' ';
	
	for(cptX=0;cptX<MAXCOLS;cptX++) colonnepleine[cptX]=false;
	
	//unsigned short cptTours;
	
	// Boucle principale (éviter les effets de bord)... 
		
	//for(cptTours=1;cptTours<=(MAXCOLS*MAXLINS);cptTours++)
	while(jetonsRouges>0 || jetonsJaunes>0) // 2021
	{
		int Colonne;
      ...
      if(!bNetworkMode)                   
      {
        printf("Entrez le numéro de la colonne [1,%d]: ",MAXCOLS);
        //scanf("%d",&Colonne);																// ELEVES: changer scanf() par getch()
				...
				}while(Colonne<1 || Colonne >MAXCOLS || !CanIPlay(Colonne,&ColonnesInterdites));
				printf("\n");
      }
      else
      {
        ...
        // BUG --> comme on ne veut pas trop se compliquer la vie on envoie un message particulier sur le réseau AVANT de fermer la socket...
				...
				// Phase plus méchante: ne pas permettre de faire un CTRL-C pendant cette période (à voir si c'est nécessaire) --> voir getch() dans uep_wide.c
    ...
    if(bNetworkMode) 
    {
      ...
      if(Colonne==-1000) 
		...
		if(IsAligned(Colonne,cptY,currentPlayer))
		{
			...
			break;																						// interdit normalement, mais bon... alternative --> jetonsJaunes=jetonsRouges=0; mais toutes les instructions des lignes suivantes seront exécutées... pfff ! 
		}
