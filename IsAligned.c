...
bool IsAligned(short lastdropx,short lastdropy,unsigned char cTarget)
{
	...
	if(cptLigne==7) // le jeton est à la base réelle du plateau de jeu
	{
		...
		count=0;
		if(tabPuissance4[cptLigne-1][cptColonne+1]==cTarget && cptColonne<7) // haut à droite et éviter "les effets de bord"
		{
	...
	count=0;						
	if(tabPuissance4[cptLigne+1][cptColonne+1]==cTarget && cptLigne<7 && cptColonne<7) // test vers le bas à droite
	{
		...
		{
			if(cptLigne+sRechercheDernierJeton>7 || cptColonne+sRechercheDernierJeton>7) 
			{
				break;
			}
		}
	...
	count=0;
	if(tabPuissance4[cptLigne+1][cptColonne-1]==cTarget && cptLigne<7 && cptColonne>0) // test vers le bas à gauche
	{
		...
		for(	sRechercheDernierJeton=0;
					...
					sRechercheDernierJeton++) // nous chercherons toujours le jeton le plus en bas (convention)
		{
			if(cptLigne+sRechercheDernierJeton>7 || cptColonne-sRechercheDernierJeton<0) 
			{
				break;
			}
		}
	...
	count=0;
	if(tabPuissance4[cptLigne-1][cptColonne+1]==cTarget && cptLigne>0 && cptColonne<7) // test vers le haut droite
	{
